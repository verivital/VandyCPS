...
bool findTarget(const cv::Mat &image, cv::Mat &blueImage) {
    ...
    cv::inRange(image, cv::Scalar(100, 20, 0), cv::Scalar(200, 100, 60),
                blueImage1); //100, 20 ,0  200, 100, 100 new1
...
int main(int argc, char **argv) {
    ...
    marker_position_data.layout.dim.push_back(
            std_msgs::MultiArrayDimension()); // set up the size and other params of the position
    ...
    ros::Publisher marker_position = nh.advertise<std_msgs::Float64MultiArray>(
            "/navigation/marker_position", 1000);
     ...
     ***********************************************************
     */
    /*raw_image = imread("/home/vaero/catkin-ws/src/cps_vision/samples/15"
                               ".jpg",IMREAD_COLOR);
    Size size(480,640);
	...
	cv::waitKey();
        CPSVision.matchPattern("/home/vaero/catkin-ws/src/cps_vision/object.jpg",blueImage);
*/
    ...
    */
    ...
    "/camera/rgb/image_raw", 1, boost::function < void(
    const sensor_msgs::ImageConstPtr &)>(boost::bind(newImageCallback, _1, &raw_image)));
    ...
    while (nh.ok()) {
        ...
        if (freshImage) {
            ...
            if (findTarget(raw_image, blueImage) && CPSVision.freshpose) {
                ...
                i_string << i;
                std::string img_path = "/home/vaero/catkin-ws/src/cps_vision/samples/blue" + i_string.str() + ".jpg";
                imwrite(img_path, blueImage);
                std::string img_path_raw = "/home/vaero/catkin-ws/src/cps_vision/samples/raw" + i_string.str() + ".jpg";
                imwrite(img_path_raw, raw_image);
                match = CPSVision.matchPattern(model_path, blueImage,i);
                ++i;
                ...
                ROS_INFO_STREAM("NO fresh image or no blue image!");
                marker_exist.data = 0;
            }
